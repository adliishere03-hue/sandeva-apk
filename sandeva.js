...
<div><strong>┃➤ Email : ${acc.email || "-"}</div>
<div><strong>┃➤ Status : ${acc.status || "-"}</div>
<div><strong>┃➤ Saldo : $${saldoStr}</div>
<div><strong>┃➤ Droplet Limit : ${dropletLimit}</div>
<div><strong>┃➤ Sisa Droplet : ${sisaDroplet} / ${dropletLimit}</div>
<div><strong>┃➤ UUID : <span style="opacity:.8">${acc.uuid || "-"}</span></div>
<div><strong>╰━━━━━━━━━━━━━━━━━━━╯</strong></div>
 `;
    }
...
async function loadDroplets() {
  ...
  try {
    ...
    // kalau ada droplet yang baru dibuat, auto-select di tabel
  ...
  if (isLastCreated) {
    pwdInfo = `
      <div class="detail-row" style="margin-top:6px;">
        ...
        <br>
        <small class="muted">
          Password ini hanya tersimpan sementara di browser kamu. Jangan lupa dicatat / disimpan.
        </small>
      </div>
    `;
  }
